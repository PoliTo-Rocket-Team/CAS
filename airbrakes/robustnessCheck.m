% Multiple Target Apogee Simulation with Delta Plot
...
apogee_min = 2800; % Minimum target apogee (m)
apogee_max = 3250; % Maximum target apogee (m)
...
for i = 1:length(target_apogees)
    ...
    fprintf('Target Apogee: %d meters, Achieved Apogee: %.2f meters, Delta: %.2f meters\n', ...
        target_apogee, final_apogee, delta_apogee);
end

%% Plot the Delta Apogee with Vertical Lines for Delta < 5
figure;
plot(target_apogees, delta_apogees, '-', 'LineWidth', 1.5, 'MarkerSize', 6); % Plot delta apogee
xlabel('Target Apogee (m)');
ylabel('Delta Apogee (Achieved - Target) (m)');
title('Delta Between Achieved and Target Apogee');
xlim([apogee_min apogee_max]);
grid on;

% Find the indices where delta_apogees is less than 5
valid_indices = find(abs(delta_apogees) < 5);

% If valid indices exist, plot the vertical lines
if ~isempty(valid_indices)
    first_valid_idx = valid_indices(1); % First index where delta < 5
    last_valid_idx = valid_indices(end); % Last index where delta < 5
    
    % Corresponding target apogees for these indices
    first_valid_apogee = target_apogees(first_valid_idx);
    last_valid_apogee = target_apogees(last_valid_idx);
    
    % Plot vertical lines at these target apogees
    xline(first_valid_apogee, '--r', 'LineWidth', 2, 'Label', 'First \Delta < 5m', 'LabelOrientation', 'horizontal');
    xline(last_valid_apogee, '--g', 'LineWidth', 2, 'Label', 'Last \Delta < 5m', 'LabelOrientation', 'horizontal');
    
    % Print to console
    fprintf('First Target Apogee with Delta < 5m: %d meters\n', first_valid_apogee);
    fprintf('Last Target Apogee with Delta < 5m: %d meters\n', last_valid_apogee);
else
    disp('No apogees achieved a delta less than 5 meters.');
end